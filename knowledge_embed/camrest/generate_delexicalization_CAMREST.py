        ...
        if (word in global_entity and word != "ask") or (i < len(words)-1 and word == "ask" and words[i+1] == 'is'):
            ...
            if rec_delex:
                ...
                if i+1 < len(words) and words[i+1] == "ly":
                    words[i+1] = ""
                    sketch_response.append(f'@{ent_type}_{str(count)}ly')
                else:
                    sketch_response.append(f'@{ent_type}_{str(count)}')
            elif i+1 < len(words) and words[i+1] == "ly":
                words[i+1] = ""
                sketch_response.append(f'@{ent_type}ly')
            else:
...
def generate_CAMREST_template(file_path, kb_file_path, rec_delex=False, verbose=False):
    ...
    out_file_path += "_record-delex" if rec_delex else "_delex"
    # with open(out_file_path + "_template.txt", "w+") as f_out_template:
    with open(f"{out_file_path}.txt", "w+") as f_out:
